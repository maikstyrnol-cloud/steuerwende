...
def build_svg_balken(info):
    ...
    bar_h, gap, label_w, bar_max_w = 32, 14, 140, 280
    total_w = label_w + bar_max_w + 70
    total_h = len(daten) * (bar_h + gap) + 60
    bars = ""
    for i, d in enumerate(daten):
        y = 40 + i * (bar_h + gap)
        w = max(4, int((d["wert"] / max_wert) * bar_max_w))
        farbe = farben.get(d.get("farbe", "blau"), "#2d6a9f")
        einheit = "%" if d["wert"] < 200 else " Mrd."
        bars += f'<text x="{label_w-8}" y="{y+bar_h//2+5}" text-anchor="end" font-size="13" fill="#4b5563" font-family="sans-serif">{d["label"][:18]}</text>'
        bars += f'<rect x="{label_w}" y="{y}" width="{w}" height="{bar_h}" fill="{farbe}" rx="2"/>'
        bars += f'<text x="{label_w+w+8}" y="{y+bar_h//2+5}" font-size="13" font-weight="bold" fill="#1a2332" font-family="sans-serif">{d["wert"]}{einheit}</text>'
    titel = info.get("titel", "")[:50]
    quelle = info.get("quelle", "")[:60]
    return f'<svg viewBox="0 0 {total_w} {total_h}" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:520px;display:block;margin:0 auto;"><text x="0" y="22" font-size="14" font-weight="bold" fill="#1a2332" font-family="sans-serif">{titel}</text>{bars}<text x="0" y="{total_h-8}" font-size="11" fill="#9ca3af" font-family="sans-serif">Quelle: {quelle}</text></svg>'
